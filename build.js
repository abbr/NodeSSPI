...
if (!force) {
  try {
    ...
    cp.execFile(process.execPath, ['quick-test.js'], (err, stdout, stderr) => {
...
function afterBuild() {
  const targetPath = path.join(__dirname, 'build', debug ? 'Debug' : 'Release', 'nodeSSPI.node');
  const installPath = path.join(__dirname, 'bin', modPath, 'nodeSSPI.node');
